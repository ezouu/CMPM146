...
def wooden_axe_for_wood(state, ID):
	# Prefer using an axe when possible; otherwise (once) create one, then chop.
	if state.wooden_axe[ID] >= 1:
		return [('op_wooden_axe_for_wood', ID)]
	# Bootstrap guard: only attempt to craft an axe once we already have some wood
	# on-hand. This prevents infinite recursion while trying to gather the first wood
	# needed for the axe's own prerequisites.
	if state.wood[ID] >= 1:
