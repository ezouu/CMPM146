...
def issue_order(state, source_planet_ID, destination_planet_ID, fleet_num_ships):
    # Python 3 safety: make sure we always send an integer ship count
    # (many starter bots compute half-ships using `/`, which yields float in py3).
    fleet_num_ships = int(fleet_num_ships)
